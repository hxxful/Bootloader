...
void BOARD_PowerConfig(void)
{
  ...
  gpio_pin_config_t pwr_config;      
   ...
   /** 底板的PWR，GPIO配置 **/       
  pwr_config.direction = kGPIO_DigitalOutput; //输出模式
  pwr_config.outputLogic =  1;                //默认高电平
  pwr_config.interruptMode = kGPIO_NoIntmode; //不使用中断
	
	IOMUXC_SetPinMux(BOARD_PWR_IOMUXC, 0U);
	IOMUXC_SetPinConfig(BOARD_PWR_IOMUXC, 0x00B0);
	GPIO_PinInit(BOARD_PWR_GPIO, BOARD_PWR_GPIO_PIN, &pwr_config);
}
